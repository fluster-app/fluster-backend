...
module.exports = {
    ...
    isNotEmpty: function isNotEmpty(obj) {
        return typeof obj !== 'undefined' && obj !== 'undefined' && obj !== null && obj.length > 0;
    },
    ...
    getDateObj: function getDateObj(obj) {
        if (this.isNull(obj)) {
            return null;
        }
    ...
    getAge: function getAge(obj) {
        if (this.isNull(obj)) {
            return 18;
        }

        var ageDifMs = Date.now() - this.getDateObj(obj).getTime();
        var ageDate = new Date(ageDifMs); // miliseconds from epoch
        return Math.abs(ageDate.getUTCFullYear() - 1970);
    },

    isParamTrue: function isBooleanParamTrue(param) {
        return !this.isStringEmpty(param) && param === "true";
    },

    convertAccentFold: function convertAccentFold(s) {
    ...
    convertAccentRegex: function convertAccentRegex(s) {
        if (this.isStringEmpty(s)) {
            return '';
        }
        ...
        for (let i=0; i<s.length; i++) {
            ...
            if ('àáâãäåÀÁÂÃÄÅaA'.indexOf(c) > -1) {
                ret += '[àáâãäåÀÁÂÃÄÅaA]';
            } else if ('æÆ'.indexOf(c) > -1) {
                ret += '[æÆ]';
            } else if ('çÇ'.indexOf(c) > -1) {
                ret += '[çÇ]';
            } else if ('èéêëÈÉÊËeE'.indexOf(c) > -1) {
                ret += '[èéêëÈÉÊËeE]';
            } else if ('ìíîïÌÍÎÏiI'.indexOf(c) > -1) {
                ret += '[ìíîïÌÍÎÏiI]';
            } else if ('ñÑ'.indexOf(c) > -1) {
                ret += '[ñÑ]';
            } else if ('òóôõöÒÓÔÕÖoO'.indexOf(c) > -1) {
                ret += '[òóôõöÒÓÔÕÖoO]';
            } else if ('œŒ'.indexOf(c) > -1) {
                ret += '[œŒ]';
            } else if ('ùúûüÙÚÛÜuU'.indexOf(c) > -1) {
                ret += '[ùúûüÙÚÛÜuU]';
            } else if ('ýÿÝŸyY'.indexOf(c) > -1) {
                ret += '[ýÿÝŸyY]';
            } else {
                ret += c;
            }
